...
const emptyWeaponReferenceResult = runFixture({
  'weapons.json': {
    knife: { requiredPassiveId: '' },
  },
});
const emptyWeaponReferenceOutput = `${emptyWeaponReferenceResult.stdout}\n${emptyWeaponReferenceResult.stderr}`;
if (emptyWeaponReferenceResult.status === 0) {
  throw new Error(`Expected content audit to reject an empty weapon passive reference.\n${emptyWeaponReferenceOutput}`);
}

if (!emptyWeaponReferenceOutput.includes('Missing requiredPassiveId in weapon knife')) {
...
const invalidBossSummonEnemyIdResult = runFixture({
  'bosses.json': {
    finalBoss: {
      id: 'finalBoss',
      enemyId: 'slime',
...
const invalidBossSummonEnemyIdOutput = `${invalidBossSummonEnemyIdResult.stdout}\n${invalidBossSummonEnemyIdResult.stderr}`;
if (invalidBossSummonEnemyIdResult.status === 0) {
  throw new Error(`Expected content audit to reject an invalid boss summon enemyId.\n${invalidBossSummonEnemyIdOutput}`);
}

if (!invalidBossSummonEnemyIdOutput.includes('Invalid enemyId in Boss config finalBoss summon: expected string')) {
  throw new Error(`Expected invalid boss summon enemyId output to identify finalBoss.\n${invalidBossSummonEnemyIdOutput}`);
}

const invalidBossSkillsResult = runFixture({
  'bosses.json': {
    finalBoss: {
      id: 'finalBoss',
      enemyId: 'slime',
...
const invalidBossSkillsOutput = `${invalidBossSkillsResult.stdout}\n${invalidBossSkillsResult.stderr}`;
if (invalidBossSkillsResult.status === 0) {
  throw new Error(`Expected content audit to reject an invalid boss skills container.\n${invalidBossSkillsOutput}`);
}

if (!invalidBossSkillsOutput.includes('Invalid skills in Boss config finalBoss: expected array')) {
  throw new Error(`Expected invalid boss skills output to identify finalBoss.\n${invalidBossSkillsOutput}`);
}

const invalidBossSummonsResult = runFixture({
  'bosses.json': {
    finalBoss: {
      id: 'finalBoss',
      enemyId: 'slime',
...
const invalidBossSummonsOutput = `${invalidBossSummonsResult.stdout}\n${invalidBossSummonsResult.stderr}`;
if (invalidBossSummonsResult.status === 0) {
  throw new Error(`Expected content audit to reject an invalid boss summons container.\n${invalidBossSummonsOutput}`);
}

if (!invalidBossSummonsOutput.includes('Invalid summons in Boss config finalBoss skill 0: expected array')) {
  throw new Error(`Expected invalid boss summons output to identify finalBoss skill 0.\n${invalidBossSummonsOutput}`);
}

const invalidBossSkillEntryResult = runFixture({
  'bosses.json': {
    finalBoss: {
      id: 'finalBoss',
      enemyId: 'slime',
...
const invalidBossSkillEntryOutput = `${invalidBossSkillEntryResult.stdout}\n${invalidBossSkillEntryResult.stderr}`;
if (invalidBossSkillEntryResult.status === 0) {
  throw new Error(`Expected content audit to reject an invalid boss skill entry.\n${invalidBossSkillEntryOutput}`);
}

if (!invalidBossSkillEntryOutput.includes('Invalid skill in Boss config finalBoss at index 0: expected object')) {
...
const invalidBossSummonEntryOutput = `${invalidBossSummonEntryResult.stdout}\n${invalidBossSummonEntryResult.stderr}`;
if (invalidBossSummonEntryResult.status === 0) {
  throw new Error(`Expected content audit to reject an invalid boss summon entry.\n${invalidBossSummonEntryOutput}`);
}

if (!invalidBossSummonEntryOutput.includes('Invalid summon in Boss config finalBoss skill 0 at index 0: expected object')) {
