...
15. Character-specific growth, deterministic weapon-tag/target-type multipliers, and reaction behavior should live in `CharacterRuntime`, `PlayerStats`, `WeaponManager`, `Enemy` target context, `PlayerController`, `PlayerHealth`, `EnemyFlow`, and character configs, not `GameScene`; character shields, heals, and temporary damage reduction should use general player survival state instead of endless-only reward state; random crit/dodge should wait for explicit RNG-backed combat work.
