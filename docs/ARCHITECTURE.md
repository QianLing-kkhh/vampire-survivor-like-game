...
Minimal Character/Stage selection UI exists. Character definitions now include initial stats, per-level growth, starting weapon, optional skin id, level-up effect, damage reaction skill, and reserved exclusive upgrade/evolution ids. The stat model can express physical, magic, projectile, aura, orbit, area, and explosion damage multipliers for future weapon-tag interactions; random crit/dodge, resource multipliers, and deeper defense hooks remain intentionally low-risk/foundation until dedicated gameplay tasks wire them in. Custom stages can appear in Stage Select after validation and local storage. Map selection, random stage selection, difficulty selection, detailed previews, unlock presentation, and custom challenge selection remain planned.
