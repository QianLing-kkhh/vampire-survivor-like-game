...
export function bestGeneralStrategyMarkdown(input: {
  ...
  const lines = [
    ...
    `- Minimum boss kill rate: ${input.strategy.searchConfig.minBossKillRate}`,
    `- Minimum p10 exp: ${input.strategy.searchConfig.minP10Exp ?? 0}`,
    `- Maximum early collapse rate: ${input.strategy.searchConfig.maxEarlyCollapseRate ?? 1}`,
    ...
    `- Avg boss damage dealt: ${input.strategy.stats.avgBossDamageDealt}`,
    `- Median boss damage dealt: ${input.strategy.stats.medianBossDamageDealt}`,
    `- P10 boss damage dealt: ${input.strategy.stats.p10BossDamageDealt}`,
    ...
    `- Early growth collapse rate: ${input.strategy.stats.earlyGrowthCollapseRate}`,
